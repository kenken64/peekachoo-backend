...
class WebSocketService {
	...
	notifyNewScore(scoreData) {
		const { userId, displayName, score, level, rank, isNewPersonalBest } =
			scoreData;
	...
	playerJoinedGame(gameId, userId, sessionId) {
		...
		console.log(
			`[WebSocket] Player ${userId} joined game ${gameId} (session: ${sessionId}). Active players: ${gamePlayers.size}`,
		);
	...
	playerLeftGame(gameId, sessionId) {
		...
		if (player) {
			...
			console.log(
				`[WebSocket] Player ${player.userId} left game ${gameId} (session: ${sessionId}). Active players: ${gamePlayers.size}`,
			);
