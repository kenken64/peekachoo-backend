...
function calculateScoreBreakdown(data) {
	...
	const territoryScore = Math.round(
		territoryPercentage * 100 * SCORE_CONFIG.TERRITORY_MULTIPLIER,
	);
	...
	const timeBonus = Math.max(
		0,
		Math.round(
			(SCORE_CONFIG.TIME_BONUS_BASE - timeTakenSeconds) *
				SCORE_CONFIG.TIME_BONUS_MULTIPLIER,
		),
	);
	...
	const levelMultiplier =
		SCORE_CONFIG.LEVEL_MULTIPLIER_BASE +
		level * SCORE_CONFIG.LEVEL_MULTIPLIER_INCREMENT;
...
async function submitScore(userId, scoreData) {
	...
	let playerStats = prepare(`SELECT * FROM player_stats WHERE user_id = ?`).get(
		userId,
	);
	...
	if (!playerStats) {
        ...
        `).run(userId);
		playerStats = prepare(`SELECT * FROM player_stats WHERE user_id = ?`).get(
			userId,
		);
	}
	...
	const currentStreak =
		livesRemaining === 3 ? (playerStats.current_streak || 0) + 1 : 1;
	...
	console.log("[ScoreService] Inserting score record:", {
		scoreId,
		level,
	...
	// Update player stats
	const newTotalScore =
		(playerStats.total_score_all_time || 0) + breakdown.totalScore;
	const newTotalLevels = (playerStats.total_levels_completed || 0) + 1;
	const newHighestLevel = Math.max(
		playerStats.highest_level_reached || 0,
		level,
	);
	const newBestStreak = Math.max(playerStats.best_streak || 0, currentStreak);
	const newBestCoverage = Math.max(
		playerStats.best_coverage || 0,
		territoryPercentage,
	);
	const newFastestLevel = Math.min(
		playerStats.fastest_level_seconds || 999999,
		timeTakenSeconds,
	);
	const newQuizCorrect =
		(playerStats.quiz_correct_total || 0) + (quizAttempts === 1 ? 1 : 0);
	const newQuizAttempts = (playerStats.quiz_attempts_total || 0) + quizAttempts;

	// Calculate new average coverage
	const totalCoverage =
		(playerStats.average_coverage || 0) * (newTotalLevels - 1) +
		territoryPercentage;
	...
	const newTotalTerritory =
		(playerStats.total_territory_claimed || 0) + territoryClaimed;
	...
	const isNewPokemon = await trackPokemonCollection(
		userId,
		pokemonId,
		territoryPercentage,
		timeTakenSeconds,
	...
	// Get session summary
	const session = prepare(`SELECT * FROM game_sessions WHERE id = ?`).get(
		sessionId,
	);
	...
	return {
		...
		rankings: {
			...
			isNewPersonalBest:
				breakdown.totalScore > (playerStats.best_single_game_score || 0),
		...
		pokemon: {
			...
			collectionCount:
				(playerStats.unique_pokemon_revealed || 0) + (isNewPokemon ? 1 : 0),
			collectionTotal: 151,
		},
	};
}
 ...
 * Track Pokemon collection
 */
async function trackPokemonCollection(
	userId,
	pokemonId,
	coverage,
	timeSeconds,
...
function startSession(userId, gameId = null) {
	...
	const existingStats = prepare(
		`SELECT 1 FROM player_stats WHERE user_id = ?`,
	).get(userId);
	if (!existingStats) {
		prepare(`
