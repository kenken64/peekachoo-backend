...
function checkAndResetMonthlySpending(userId) {
	...
	const now = new Date();
	const resetDate = user.purchase_reset_date
		? new Date(user.purchase_reset_date)
		: null;
...
exports.checkPurchaseStatus = async (req, res) => {
	try {
		...
		if (userStatus.first_purchase_date && !purchaseResetDate) {
			...
			prepare(`UPDATE users SET purchase_reset_date = ? WHERE id = ?`).run(
				purchaseResetDate,
				req.user.id,
			);
			saveDatabase();
...
exports.createOrder = async (req, res) => {
	if (!razorpay) {
		return res
			.status(503)
			.json({ error: "Payment service unavailable (configuration error)" });
	}
	try {
		...
		const userInfo = prepare("SELECT username FROM users WHERE id = ?").get(
			req.user.id,
		);
		...
		if (generated_signature === razorpay_signature) {
			...
			if (!qty && razorpay && razorpay_order_id) {
				try {
					...
					qty = parseInt(order.notes?.quantity, 10) || 1;
			...
			console.log(
				`Processing valid payment ${razorpay_payment_id} for user ${req.user.id}, qty: ${qty}, cost: SGD ${costSGD}`,
			);

			// Get current user data
			const currentUser = prepare(
				"SELECT first_purchase_date, purchase_reset_date FROM users WHERE id = ?",
			).get(req.user.id);
			...
			const firstPurchaseDate =
				...
				currentUser.first_purchase_date.length > 0
					? currentUser.first_purchase_date
					: now;
			...
			console.log(
				`Setting first_purchase_date: ${firstPurchaseDate}, purchase_reset_date: ${purchaseResetDate}`,
			);
			...
			const user = prepare(
				"SELECT shields, monthly_spent, purchase_reset_date FROM users WHERE id = ?",
			).get(req.user.id);
...
exports.handleWebhook = async (req, res) => {
	try {
		...
		const isValid = RazorpayClass.validateWebhookSignature(
			body,
			signature,
			secret,
		);
...
exports.debugSyncData = async (_req, res) => {
	try {
		// Get all purchases with raw created_at
		const allPurchases = prepare(
			`SELECT user_id, quantity, amount_sgd, created_at FROM purchases`,
		).all();

		// Get user data
		const userData = prepare(
			`SELECT id, username, monthly_spent, purchase_reset_date, first_purchase_date FROM users WHERE total_spent > 0 OR id IN (SELECT DISTINCT user_id FROM purchases)`,
		).all();
...
exports.syncRazorpayPayments = async (_req, res) => {
	if (!razorpay) {
	...
	try {
		...
		try {
			while (true) {
				...
				console.log(
					`[Razorpay Sync] Batch received: ${batch.items?.length || 0} orders`,
				);
			...
			return res.status(500).json({
				error: `Failed to fetch orders from Razorpay: ${fetchError.message}`,
			});
		}

		results.fetched = orders.length;
		console.log(
			`[Razorpay Sync] Fetched ${orders.length} orders from Razorpay`,
		);
		...
		for (const order of orders) {
			try {
				...
				try {
					...
					if (payments.items && payments.items.length > 0) {
						...
						if (capturedPayment) {
							...
							try {
								...
								console.warn(
									`[Razorpay Sync] Could not fetch payment details for ${paymentId}:`,
									pErr.message,
								);
							}
						}
					...
					console.warn(
						`[Razorpay Sync] Could not fetch payments for order ${orderId}:`,
						e.message,
					);
				}
				...
				if (existingPurchase) {
					...
					if (
						...
						Math.abs(existingPurchase.amount_sgd - amountSGD) > 0.01 ||
						existingPurchase.status !== paymentStatus
					) {
						results.updated++;
						console.log(
							`[Razorpay Sync] Updated purchase ${orderId}: qty=${quantity}, amt=${amountSGD}, status=${paymentStatus}, settlement=${settlementId}`,
						);
					} else {
						results.skipped++;
					}
					...
					results.created++;
					console.log(
						`[Razorpay Sync] Created purchase ${paymentId}: user=${userId}, qty=${quantity}, amt=${amountSGD}, status=${paymentStatus}`,
					);
				}
			} catch (e) {
				console.error(
					`[Razorpay Sync] Error processing order ${orderId}:`,
					e.message,
				);
		...
		const usersWithPurchases = prepare(
			`SELECT DISTINCT user_id FROM purchases`,
		).all();
		console.log(
			`[Razorpay Sync] Found ${usersWithPurchases.length} users with purchases:`,
		...
		console.log(
			`[Razorpay Sync] Total affected users to recalculate: ${affectedUserIds.size}`,
		);
		...
		for (const userId of affectedUserIds) {
			...
			try {
				...
				const firstPurchaseDate =
					firstPurchase?.created_at || new Date().toISOString();
				...
				console.log(
					`[Razorpay Sync] User ${userId}: checking purchases between ${currentMonthStart} and ${nextMonthStart}`,
				);

				// Debug: List all purchases for this user
				const allPurchases = prepare(
					`SELECT created_at, amount_sgd FROM purchases WHERE user_id = ?`,
				).all(userId);
				console.log(
					`[Razorpay Sync] User ${userId} purchases:`,
				...
				console.log(
					`[Razorpay Sync] User ${userId} monthly totals:`,
				...
				// Update user totals
				console.log(
					`[Razorpay Sync] Updating user ${userId}: shields=${totalShields}, totalSpent=${totalSpent}, monthlySpent=${monthlySpent}, resetDate=${purchaseResetDate}`,
				);
				...
				console.log(
					`[Razorpay Sync] Recalculated user ${userId}: shields=${totalShields}, spent=${totalSpent}, monthly=${monthlySpent}, resetDate=${purchaseResetDate}`,
				);
			} catch (e) {
				console.error(
					`[Razorpay Sync] Error recalculating user ${userId}:`,
					e.message,
				);
		...
		console.log(
			`[Razorpay Sync] Complete: fetched=${results.fetched}, created=${results.created}, updated=${results.updated}, skipped=${results.skipped}, usersRecalculated=${results.usersRecalculated}`,
		);
