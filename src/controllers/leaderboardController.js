...
async function getGlobalLeaderboard(req, res, next) {
	try {
		...
		switch (period) {
			...
			case "weekly":
				...
				periodStart.setUTCDate(
					periodStart.getUTCDate() - periodStart.getUTCDay(),
				);
				periodStart.setUTCHours(0, 0, 0, 0);
				dateFilter = `AND ps.created_at >= '${formatForSQLite(periodStart)}'`;
				break;
		...
		let _orderBy = "total_score DESC";
		if (sortBy === "level") _orderBy = "highest_level DESC, total_score DESC";
		if (sortBy === "streak") _orderBy = "best_streak DESC, total_score DESC";
...
async function getAroundMe(req, res, next) {
	try {
		...
		const rangeNum = Math.min(Math.max(1, parseInt(range, 10) || 5), 10);
...
async function submitScore(req, res, next) {
	try {
		...
		if (result.pokemon.isNewReveal) {
			websocketService.notifyPokemonReveal(
				userId,
				{
		...
		if (streakBonus > 0) {
			websocketService.notifyStreakMilestone(
				userId,
				result.session.currentStreak,
				streakBonus,
			);
		}
...
async function endSession(req, res, next) {
	try {
		...
		const sessionInfo = prepare(
			`SELECT game_id FROM game_sessions WHERE id = ?`,
		).get(sessionId);
