	...
	try {
		...
		res.json({
			...
			data: {
				...
				...(isOwner
					? {
							activePlayerCount: websocketService.getActivePlayerCount(
								id,
								userId,
							),
						}
...
exports.togglePublish = async (req, res) => {
	try {
		...
		if (isCurrentlyPublished) {
			const activePlayerCount = websocketService.getActivePlayerCount(
				id,
				userId,
			);
		...
		prepare(
			"UPDATE games SET is_published = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
		).run(newStatus, id);
...
exports.incrementPlayCount = async (req, res) => {
	try {
		...
		prepare("UPDATE games SET play_count = play_count + 1 WHERE id = ?").run(
			id,
		);
