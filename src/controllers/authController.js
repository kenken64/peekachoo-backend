...
exports.checkUsername = async (req, res) => {
	try {
		...
		const user = prepare("SELECT id FROM users WHERE username = ?").get(
			username,
		);
...
exports.startRegistration = async (req, res) => {
	try {
		...
		).run(challengeId, user.id, options.challenge, "registration", expiresAt);
...
exports.completeRegistration = async (req, res) => {
	try {
		...
		console.log("[Auth] Storing credential:", {
			credentialId,
			userId,
			counter,
		});
...
exports.startAuthentication = async (req, res) => {
	try {
		...
		if (username) {
			// Get user
			const user = prepare("SELECT * FROM users WHERE username = ?").get(
				username,
			);
			if (!user) {
		...
		prepare(
			"INSERT INTO challenges (id, user_id, challenge, type, expires_at) VALUES (?, ?, ?, ?, ?)",
		).run(challengeId, userId, options.challenge, "authentication", expiresAt);
...
exports.completeAuthentication = async (req, res) => {
	try {
		...
		let credential = prepare("SELECT * FROM credentials WHERE id = ?").get(
			response.id,
		);
		...
		if (!credential) {
			console.log(
				"[Auth] Credential not found. Available credentials:",
		...
		if (
			...
			credential.user_id !== challengeRecord.user_id
		) {
			prepare("DELETE FROM challenges WHERE id = ?").run(challengeId);
			return res.status(400).json({
				error:
					"Wrong passkey selected. Please select the passkey for the username you entered.",
			});
		}
		...
		// Get user and generate token
		const user = prepare("SELECT * FROM users WHERE id = ?").get(
			credential.user_id,
		);
		const token = generateToken(user);
		...
		// Get user level
		const stats = prepare(
			"SELECT highest_level_reached FROM player_stats WHERE user_id = ?",
		).get(user.id);
...
exports.getCurrentUser = async (req, res) => {
	try {
		const user = prepare(
			"SELECT id, username, display_name, shields FROM users WHERE id = ?",
		).get(req.user.id);
		if (!user) {
...
exports.purchaseShield = async (req, res) => {
	try {
		...
		const user = prepare("SELECT shields FROM users WHERE id = ?").get(
			req.user.id,
		);
...
exports.consumeShield = async (req, res) => {
	try {
		...
		prepare("UPDATE users SET shields = shields - 1 WHERE id = ?").run(
			req.user.id,
		);
		saveDatabase();

		const updatedUser = prepare("SELECT shields FROM users WHERE id = ?").get(
			req.user.id,
		);
