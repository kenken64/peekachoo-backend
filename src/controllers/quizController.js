...
exports.generateQuiz = async (req, res) => {
	try {
		...
		if (isJP) {
			try {
				const dbResult = prepare(
					"SELECT name_jp FROM pokemon WHERE LOWER(name) = ?",
				).get(cleanPokemonName.toLowerCase());
				if (dbResult?.name_jp) {
			...
			try {
				const dbResult = prepare(
					"SELECT name_cn FROM pokemon WHERE LOWER(name) = ?",
				).get(cleanPokemonName.toLowerCase());
				if (dbResult?.name_cn) {
					...
					console.log(
						`[Quiz] CN name not in DB for ${cleanPokemonName}, trying API...`,
					);
					// Fallback: Try to fetch from PokeAPI if not in DB
					// Try Simplified Chinese first, then Traditional
					let apiName = await fetchPokemonNameFromApi(
						cleanPokemonName,
						"zh-Hans",
					);
					if (!apiName) {
						apiName = await fetchPokemonNameFromApi(
							cleanPokemonName,
							"zh-Hant",
						);
					}
					...
					if (apiName) {
						...
						console.warn(
							`[Quiz] Failed to find CN name for ${cleanPokemonName}`,
						);
					}
				}
			} catch (e) {
...
async function fetchPokemonNameFromApi(pokemonName, langCode) {
	try {
		...
		const nameObj = response.data.names.find(
			(n) => n.language.name === langCode,
		);
		return nameObj ? nameObj.name : null;
	} catch (error) {
		console.warn(
			`Failed to fetch ${langCode} name for ${pokemonName} from API:`,
			error.message,
		);
		return null;
	}
...
function generateWrongAnswers(
	...
	if (isJP) {
		try {
			const excludeNames = [
				...
				...availableNames.map((n) => n.toLowerCase()),
			];
	...
	if (availableNames.length < 3) {
		try {
			...
			console.error(
				"[Quiz] Failed to fetch Pokemon from database:",
				dbError.message,
			);
			...
			const fallbackFiltered = fallbackPokemon.filter(
				(name) =>
					name.toLowerCase() !== correctAnswer.toLowerCase() &&
					!availableNames.some((n) => n.toLowerCase() === name.toLowerCase()),
			);

			availableNames = [
				...availableNames,
				...shuffleArray(fallbackFiltered).slice(0, 3 - availableNames.length),
			];
		}
	}
